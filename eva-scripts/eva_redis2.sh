...
function run_test() {
	...
	output=$($bench -s $IP -p $PORT -c $CONNS_PER_THREAD -n $SREQS_PER_CLI -t $THREADS \
		    -d $DSIZE --key-pattern $PATT --ratio 1:0 -P redis --hide-histogram --pipeline 4 2>&1 \
	...
	# get
	output=$($bench -s $IP -p $PORT -c $CONNS_PER_THREAD -n $GREQS_PER_CLI -t $THREADS \
                    -d $DSIZE --key-pattern $PATT --ratio 0:1 -P redis --hide-histogram --pipeline 4 2>&1 \
                    | grep -E "Type|---|Sets|Gets|Totals")
