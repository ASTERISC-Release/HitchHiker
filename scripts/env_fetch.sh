...
tf_fetch() {
    ...
    if [ ! -d $TF_A_DIR ]; then
        ...
        # curl $TF_A_SRC | tar -xz
        # if [ $? -ne 0 ]; then
        #     log_error "[TF-A] Error downloading TF-A."
        #     exit
        # fi
        ln -s $SRC_DIR/$TF_A_DIR $THIRD_PARTY_DIR/$TF_A_DIR
    fi

    if [ ! -d $TF_EVA_DIR ]; then
        log "===> Soft linking TF-A-EVA (Trusted Firmware)... from src/"
        ln -s $SRC_DIR/$TF_EVA_DIR $THIRD_PARTY_DIR/$TF_EVA_DIR
    fi
    popd
}
...
fvp_model_fetch() {
    ...
    if [ ! -d Base_RevC_AEMvA_pkg ]; then
        ...
        # tar xzvf FVP_Base_RevC-2xAEMvA_11.19_14_Linux64.tgz
        # rm FVP_Base_RevC-2xAEMvA_11.19_14_Linux64.tgz
    fi
}
...
third_parties_fetchall() {
    ...
    prerequisite_fetch
    # 4. download trusted firmware 
    tf_fetch
    # 5. download the arm development workspace
    dev_workspace_fetch
}
...
if [ $# != 1 ]; then
    log_error "Usage: ./env_fetch.sh [all | cross_compile | prerequisite | fvp_model | tf]"
    exit
fi
