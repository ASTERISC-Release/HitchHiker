...
dev_workspace_build_linux() {
    ...
    if [[ -d $LINUX_OUT_DIR && ($1 == "build" || $1 == "all") ]]; then
        ...
        ./build-scripts/build-linux.sh -p $DEV_WORKSPACE_PLATFORM -f $DEV_WORKSPACE_FS package
    fi
}
...
__print_usage()
{
	log "Usage: env_build.sh -s <software> -p [juno|fvp] <command>"
	log
	log "env_build.sh: Builds the platform software stack with the"
	...
	# log
	# log "Example 1: ./build-scripts/build-test-uefi.sh -p aemfvp-a all"
	# log "    This command builds the required software components of the aemfvp-a"
	# log "    platform to boot upto the EFI shell."
	# log
	# log "Example 2: ./build-scripts/build-test-uefi.sh -p aemfvp-a clean"
	# log "    This command cleans the previous build of the aemfvp-a platform"
	# log "    software stack."
	log
	exit 0
}
