...
static unsigned long hhkrd_secIO_issuer(int fd, hhkr_scatterlist_pgs_t *sg_pgs) {
    ...
    printf("[hhkrd] _activate_user: 0x%lx, _paddr_lo_32: 0x%x.\n", 
            (unsigned long)_activate_user, (unsigned int)((_paddr) & 0xffffffff));
    printf("[hhkrd] _activate_user + 4: 0x%lx, _paddr_hi_32: 0x%x.\n", 
            (unsigned long)(_activate_user + 4), (unsigned int)((_paddr) >> 32));

    asm volatile("dsb st":::"memory");
    *(unsigned int *)_activate_user = (unsigned int)((_paddr) & 0xffffffff);
    asm volatile("dsb st":::"memory");
    *(unsigned int *)(_activate_user + 4) = (unsigned int)((_paddr) >> 32);
    asm volatile("dsb st":::"memory");
}
/* ----------------------------------------- sec IO end ------------------------------------------------------- */
...
int main(int argc, char *argv[]) {
    ...
    if (ret) {
        ...
        _mmio_virt = mmap(NULL, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, devmem_fd, *activate_phys);
