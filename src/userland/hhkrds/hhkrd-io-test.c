...
static atomic_ulong obs_blk_starts[4] = {0, 2097152, 4194304, 6291456};

hhkr_scatterlist_pgs_t *hhkr_obs_sg_pgs[__HHKR_OBS_MAX_ID][2];

/* current scatter list buffer to write to */
static atomic_int hhkr_current_sg_pg_ids[__HHKR_OBS_MAX_ID];
/* store pending scatterlist buffer */
static atomic_int hhkr_tostore_sg_pg_ids[__HHKR_OBS_MAX_ID];

/* format of <hhkr_secIO_pending_job>:
 * [int] pending_job
 * [unsigned long] paddr (dma)
 * [unsigned long] activate (mmio) mmaped user addr
 * [unsigned long] activate (mmio) phys addr
 */
int *hhkr_secIO_pending_job = NULL;
unsigned long *paddr = NULL;
unsigned long *activate = NULL;
unsigned long *activate_phys = NULL;

void *_mmio_virt = NULL;
volatile void *mmio_reg = NULL;
...
static unsigned int hhkr_write_to_sg_pg(int obs_type, void *data, unsigned int size, int is_full) {
    int cur_idx = atomic_load(&hhkr_current_sg_pg_ids[obs_type]);
    hhkr_scatterlist_pgs_t *cur_sg_pg = hhkr_obs_sg_pgs[obs_type][cur_idx];
    /* sanity check! */
    if (is_full == 1 && cur_sg_pg->cur_pos != 0) {
        printf("[hhkrd] error: the freshed sg_pg (idx: %d) for obs[%d] is not empty.\n",
               cur_idx, obs_type);
    }

    /* enough space, we put them into this buffer */
    ...
    /* we should flush them into disk now */
    /* mark as to_store for the store thread */
    if (atomic_load(&hhkr_tostore_sg_pg_ids[obs_type]) != -1) {
        /* has pending tostore? error now */
        printf("[hhkrd] error: has pending tostore sg_pg (idx: %d) for obs[%d].\n",
               hhkr_tostore_sg_pg_ids[obs_type], obs_type);
    }
    atomic_store(&hhkr_tostore_sg_pg_ids[obs_type], cur_idx);
    /* switch current sg_pg for the next buffer */
    atomic_store(&hhkr_current_sg_pg_ids[obs_type], (cur_idx + 1) % 2);
    /* write again */
    return hhkr_write_to_sg_pg(obs_type, data, size, 1);
}

static unsigned long hhkrd_secIO_issuer(int fd, hhkr_scatterlist_pgs_t *sg_pgs) {
    sg_pgs->write_size = sg_pgs->buf_size;
    /* 1. notify monitor to configure scatterlist, prb, and sge */
    ioctl(fd, HHKRD_ASSIGN_SECIO, sg_pgs->pgs_kern_addr);
    /* 2. MMIO */
    // void *_activate_user = (void *)*activate;
    unsigned long _paddr = *paddr;

    // debug
    printf("[hhkrd] _activate_user: 0x%lx, _paddr_lo_32: 0x%x.\n", 
            (unsigned long)mmio_reg, (unsigned int)((_paddr) & 0xffffffff));
    printf("[hhkrd] _activate_user + 4: 0x%lx, _paddr_hi_32: 0x%x.\n", 
            (unsigned long)(mmio_reg + 4), (unsigned int)((_paddr) >> 32));

    asm volatile("dsb st":::"memory");
    *(volatile unsigned int *)mmio_reg = cpu_to_le32((unsigned int)((_paddr) & 0xffffffff));
    asm volatile("dsb st":::"memory");
    *(volatile unsigned int *)(mmio_reg + 4) = cpu_to_le32((unsigned int)((_paddr) >> 32));
    asm volatile("dsb st":::"memory");
}
/* ----------------------------------------- sec IO end ------------------------------------------------------- */
...
static void __debug_print_policy_time_statistics(hhkr_buf_t *mbuf) {
...
void *store_thread(void *arg) {
    ...
    sleep_time.tv_sec = 0;
    sleep_time.tv_nsec = 100 * NS_TO_MS;   // sleep 200ms
    while(1) {
        /* check if there is sg_pg to store */
        for (int i = 0; i < 1 /* only have bpfs observability to store now... */; i++) {
            int tostore_idx = atomic_load(&hhkr_tostore_sg_pg_ids[i]);

            if (tostore_idx != -1) {
                /* store the sg_pg */
                hhkr_scatterlist_pgs_t *sg_pg = hhkr_obs_sg_pgs[i][tostore_idx];
                unsigned long blk_start = atomic_load(&obs_blk_starts[i]);

                sg_pg->blk_start = blk_start; 
                sg_pg->write_size = sg_pg->buf_size;
                ...
                atomic_store(&hhkr_tostore_sg_pg_ids[i], -1);
                /* update blk_start */
                atomic_store(&obs_blk_starts[i], blk_start + (sg_pg->buf_size) / SATA_BLK_SZ);
            }
        }
        /* sleep */
...
int main(int argc, char *argv[]) {
    ...
    /* mmap secIO pending job status */
    ret = (unsigned long)ioctl(fd, HHKRD_MMAP_SECIO_STAT, 0);
    if (ret) {
        hhkr_secIO_pending_job = (int *)(ret << 12);
        paddr = (unsigned long *)((void *)hhkr_secIO_pending_job + sizeof(int));
        activate = (unsigned long *)((void *)paddr + sizeof(unsigned long));
        activate_phys = activate + 1;
        printf("<secIO_pending_status> addr: 0x%lx.\n", (unsigned long)hhkr_secIO_pending_job);
        printf("<padr> addr: 0x%lx. val (dma): 0x%lx\n", 
                (unsigned long)paddr, *paddr);
        printf("<activate> addr: 0x%lx. val (mmio useraddr): 0x%lx\n", 
                (unsigned long)activate, *activate);
        printf("<activate_phys> addr: 0x%lx. val (mmio physaddr): 0x%lx\n", 
                (unsigned long)activate_phys, *activate_phys);
        ...
        unsigned long _off = *activate_phys & 0x0fff;
        _mmio_virt = mmap(NULL, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, devmem_fd, (*activate_phys & 0xfffff000));
        if (_mmio_virt == MAP_FAILED) {
            perror("Error: cannot mmap mmio virt.\n");
            return 1;
        }
        mmio_reg = (volatile uint32_t*)((uintptr_t)_mmio_virt + _off);
        printf("mmaped mmio virt addr: 0x%lx. mmio_reg addr: 0x%lx.\n", 
                (unsigned long)_mmio_virt, (unsigned long)mmio_reg);
    }
    ...
    hhkrd_secIO_issuer(fd, hhkr_obs_sg_pgs[0][0]);
    ...
    if (pthread_create(&store_t, NULL, store_thread, NULL) != 0) {
        perror("Error: cannot create store thread.\n");
        return 1;
    }
    ...
    while (1) {
        /* receive message (a just-secured buffer) from queue */
        hhkr_receive_message(&buf);
        
        /* process message */
#if defined(QUIET)
        /* consume the buffer into the store (sg_pg) buffer */
        w_size = hhkr_write_to_sg_pg(buf.obs_type, (void *)(buf.user_addr), buf.cur_pos, 0);
#else
