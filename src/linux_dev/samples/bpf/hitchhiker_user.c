...
static void print_help(char* progname) {
    printf(
        ...
        "  --interval,  -i [MICROSECONDS]   Set the hitchhiker secure policy interval to [MICROSECONDs] us (default 1000us).\n"
        ...
        "  --omnilog,   -m                  Enable OmniLog mode.\n"
        "  --memcpy,    -x                  Enable el3-memcpy mode for buffer protection instead of GPT permission change.\n"
...
int main(int argc, char *argv[]) {
    ...
    struct option long_options[] = {
        ...
        {"memcpy", optional_argument, NULL, 'x'},
    ...
    while ((c = getopt_long(argc, argv, "i::t::c::p::x::adehmns", long_options, &option_index)) != -1) {
        switch (c) {
            case 't':
                if(optarg) {
            ...
            case 'x':
                if (optarg) {
                    el3_memcpy_mode = atoi(optarg);
                }
                LOG_INFO("EL3-memcpy mode: %d.", el3_memcpy_mode);
                break;
            case 's':
    ...
    const char *file = "./hitchhiker_kern.o";
