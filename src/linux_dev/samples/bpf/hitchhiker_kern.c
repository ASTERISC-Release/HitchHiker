...
static __always_inline void debug_print(hhkr_audit_log_t *audit_data) {
    bpf_printk("[Syscall Log] prog: %s, tid: %u, pid: %u.\n", 
                audit_data->ctx.comm, audit_data->ctx.tid, audit_data->ctx.pid);
...
int tracepoint__raw_syscalls__sys_enter(struct bpf_raw_tracepoint_args *ctx) {
    ...
    if (enable_audit_log()) {
        ...
        if (sysid == SYS_EXIT || sysid == SYS_EXIT_GROUP || sysid == SYS_RT_SIGRETURN) { // no ret syscalls
            // send value
            if (enable_omnilog()) {
                /* evaluation compare mode */
                bpf_write_omni_buf_k((void *)&hhkr_audit_data, sizeof(hhkr_audit_log_t));
            }
            if (enable_hhkrd()) {
                bpf_write_current_buf(0, (void *)&hhkr_audit_data, sizeof(hhkr_audit_log_t));
            }
            update_log_count(0);
        }
    }
    ...
    if (enable_app_log()) {
        if (hhkr_should_app_log(&hhkr_audit_data)) {
            ...
            if (enable_omnilog()) {
                /* evaluation compare mode */
                bpf_write_omni_buf_k((void *)&app_log, sizeof(hhkr_app_log_t));
            }
            if (enable_hhkrd()) {
                bpf_write_current_buf(0, (void *)&app_log, sizeof(hhkr_app_log_t));   
            }
            update_log_count(1);
        }
    }
...
int tracepoint__raw_syscalls__sys_exit(struct bpf_raw_tracepoint_args * ctx) {
    ...
    if (enable_net_log()) {
        ...
        if (hhkr_config_net_log(hhkr_audit_data, &hhkr_net_log)) {
            if (enable_omnilog()) {
                bpf_write_omni_buf_k((void *)&hhkr_net_log, sizeof(hhkr_net_log_t));
            }
            if (enable_hhkrd()) {
                bpf_write_current_buf(0, (void *)&hhkr_net_log, sizeof(hhkr_net_log_t));
            }
            update_log_count(2);
        }   
    }
