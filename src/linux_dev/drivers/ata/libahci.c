...
extern void set_debug_replay_sata(bool);
#endif
...
#if defined(DEBUG_RECORD_SATA) && defined(CONFIG_HITCHHIKER)
	if (fetch_debug_record_sata()) {
		printk(KERN_INFO "[libahci.c][ahci_fill_cmd_slot]tag=%d, opts=0x%x (le32: 0x%x).\n", tag, opts, cpu_to_le32(opts));
		if (pp->active_link && pp->active_link->ap) {
			printk(KERN_INFO "[libahci.c][ahci_fill_cmd_slot]port_no: %d, pp->cmd_tbl_dma: 0x%llx (le32: 0x%lx), cmd_slot_base: %lx.\n",
					pp->active_link->ap->port_no, pp->cmd_tbl_dma, (unsigned long)cpu_to_le32(pp->cmd_tbl_dma), (unsigned long)pp->cmd_slot);
		}
		else {
			printk(KERN_INFO "[libahci.c][ahci_fill_cmd_slot]pp->cmd_tbl_dma: 0x%llx (le32: 0x%lx), cmd_slot_base: %lx. (pp no active link.)\n",
					pp->cmd_tbl_dma, (unsigned long)cpu_to_le32(pp->cmd_tbl_dma), (unsigned long)pp->cmd_slot);
		}
	}
#endif
}
...
#if defined(CONFIG_HITCHHIKER) && defined(DEBUG_RECORD_SATA)
		/* record sg information and debug the content */	
		if (fetch_debug_record_sata()) {
			/* is this last sg? */
			printk(KERN_INFO "[libahci.c][ahci_fill_sg] qc->n_elem: 0x%x, sg_off: 0x%x, sg_dma_addr: 0x%llx, sg_len: 0x%x, sg_dma_len: 0x%x, sg_is_last_flag: %ld.\n",
								 qc->n_elem, sg->offset, addr, sg->length, sg_len, sg_is_last(sg));
			// struct page *sg_pg = sg_page(sg);
			// convert sg_page to kernel readable virtual address
			// unsigned long sg_virt = (unsigned long)page_address(sg_pg);
			// printk(KERN_INFO "[libahci.c][ahci_fill_sg]sg cont: %s.\n", (char *)sg_virt);
		}
#endif
...
#if defined(DEBUG_RECORD_SATA) && defined(CONFIG_HITCHHIKER)
	if (fetch_debug_record_sata())
		printk(KERN_INFO "[libahci.c][ahci_qc_prep]is_atapi: %d, opts: 0x%x, pmp: %d, qc->flags: 0x%lx.\n",
		       is_atapi, opts, qc->dev->link->pmp, qc->flags);
#endif
...
static irqreturn_t ahci_single_level_irq_intr(int irq, void *dev_instance)
{
	...
	spin_lock(&host->lock);
#if defined(CONFIG_HITCHHIKER) && defined(DEBUG_RECORD_SATA)
	/* hacky way to finish the replay is instrumentation :) */
	printk(KERN_INFO "[libahci.c][ahci_single_level_irq_intr] triggered IRQ: %d. current pid: %d, irq_stat: %x, irq_masked: %x.\n",
					  irq, current->pid, irq_stat, irq_masked);
	
	/* bypass all normal handlers */
	if (hhkr_secIO_pending_job && *hhkr_secIO_pending_job) {
		*hhkr_secIO_pending_job = 0;
		/* find the ata port of /dev/sda */
		ap = host->ports[0];
		port_mmio = ahci_port_base(ap); // phys addr 0x812c0100 or 0x812?0100 more general
		status = readl(port_mmio + PORT_IRQ_STAT);
		writel(status, port_mmio + PORT_IRQ_STAT);
		...
		// debug
		printk(KERN_INFO "[libahci.c][ahci_single_level_irq_intr] hacky replay for irq num: %d now.\n", irq);
		return IRQ_RETVAL(1);
	}
#endif
...
// RECORD PORT START: pp->cmd_tbl; pp->cmd_slot
static int ahci_port_start(struct ata_port *ap)
{
	struct ahci_host_priv *hpriv = ap->host->private_data;
