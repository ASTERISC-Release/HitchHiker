...
#if defined(CONFIG_HITCHHIKER) && defined(DEBUG_RECORD_SATA)
	if (fetch_debug_record_sata()) {
		log_info("block: 0x%llx, n_block: 0x%x, tf_flags: %u, tag: 0x%x, class: 0x%x, dev->no: %d, dev->ap->no: %d.\n",
				block, n_block, tf_flags, tag, class, dev->devno, dev->link->ap->port_no);
	}
#endif
...
#if defined(CONFIG_HITCHHIKER) && defined(DEBUG_RECORD_SATA)
	if (fetch_debug_record_sata()) {
		log_info("dma_mapped_sg: qc->n_elem: %d, qc->dma_dir: %d, ret n_elem: %d.\n",
				  qc->n_elem, qc->dma_dir, n_elem);
	}
#endif
