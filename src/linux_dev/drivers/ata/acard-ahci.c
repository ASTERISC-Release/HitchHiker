...
static void acard_ahci_qc_prep(struct ata_queued_cmd *qc)
{
	...
	printk(KERN_INFO "[acard-ahci.c][acard_ahci_qc_prep]: pp->cmd_tbl: 0x%lx, cmd_tbl: 0x%lx, qc->dev->link->pmp: 0x%x, pp->active_link->pmp: 0x%x\n", 
			(unsigned long)pp->cmd_tbl, (unsigned long)cmd_tbl, qc->dev->link->pmp, pp->active_link->pmp);
#endif
