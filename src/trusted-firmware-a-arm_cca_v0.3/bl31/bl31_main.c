...
void bl31_main(void)
{
	...
	/* debug print MDCR registers */
	uint64_t mdcr_el2, mdcr_el3;
	asm volatile("MRS %0, MDCR_EL2\n" : "=r" (mdcr_el2));
	asm volatile("MRS %0, MDCR_EL3\n" : "=r" (mdcr_el3));
	NOTICE("BL31: MDCR_EL2 = 0x%llx, MDCR_EL3 = 0x%llx\n", mdcr_el2, mdcr_el3);
	...
	plat_ic_set_interrupt_type(49152, U(0));
	unsigned int type = plat_ic_get_interrupt_type(49152);
	NOTICE("IRQ NUM 49152 plat_ic_get_interrupt_type: %d\n", type);
