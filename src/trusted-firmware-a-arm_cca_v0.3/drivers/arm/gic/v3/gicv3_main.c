...
{
#if ENABLE_HHKRD
	NOTICE("Trigger gicv3_get_interrupt_type for id: %u.\n", id);
#endif
	...
	if (is_sgi_ppi(id)) {
		...
		NOTICE("gicv3_get_interrupt_type: gicr_base = 0x%lx, id = %d, igroup = %d, grpmodr = %d.\n", 
				gicr_base, id, igroup, grpmodr);
	} else {
		...
		unsigned int type)
{
	...
	if (is_sgi_ppi(id)) {
		/* For SGIs: 0-15, PPIs: 16-31 and EPPIs: 1056-1119 */
		gicr_base = gicv3_driver_data->rdistif_base_addrs[proc_num];
		NOTICE("gicv3_set_interrupt_type: gicr_base = 0x%lx, id = %d, type = %d, igroup = %d, grpmod = %d.\n",
		        gicr_base, id, type, igroup, grpmod);
